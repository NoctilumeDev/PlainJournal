...
> 完成顾客端、管理端与特殊页面的第一轮前端轻量化重构，建立可独立演进的页面层级、
> 响应式基线和视觉文档门禁；不增加业务域，不修改数据库结构、交易状态机、公开 API
> 或后端事实所有权。

### Added

- 发布功能模块图与系统架构图的独立渲染页面、响应式布局和 GitHub Pages 入口；
- 建立 33 条真实路由注册表、66 份桌面/移动基线截图、四张联络表及对应 CI 漂移检查；
- 为管理端提供只承载布局的 `ListWorkbench` 与 `SplitWorkbench`，不在共享层复制页面
  状态、字段、命令或恢复流程。

### Changed

- 顾客端 20 条真实路由完成发现、搜索、商品、购物袋、结算、订单、账户、通知和客服
  旅程的第一轮轻量化布局；
- 管理端 13 条真实路由完成目录、履约、售后、库存、营销、治理、客服和评价工作台的
  桌面与移动端第一轮布局；
- 旧的整页强压方案退出当前实现依据，页面改由轻底板、页面家族、纯布局器和业务切片
  分层组合；特殊页面统一为无多余嵌套的状态语法。
