...
M7 分布式 ID 只作为有限领域代表实现：当前仅替换 `trade_order.id`，内部快照和历史
行不为统一形式机械迁移。worker 唯一性由 MySQL 租约裁决，Redis 不参与最终正确性。
...
只删除归档副本的回滚和回滚后重放已在两个真实 MySQL 上通过。
...
真实 MySQL 和四个 schema，且没有反向复制，因此不描述为生产无停机 CDC。
...
- Analytics Java 服务是当前自营 B2C 运营统计主线，只消费版本化事件并持有可重建
  读模型。PlainJournalPro 的 Go 服务必须承担独立、可验证的吞吐或连接职责，不提前
  替代 Java 统计，也不参与当前订单、支付或库存裁决。
...
- 多商户、店铺、子订单、分账和结算进入独立 PlainJournalPro，不在当前仓库追加
  `tenant_id` 或零散商户判断。Go 的最终职责由所有权、容量和失败边界决定，不能只为
  展示语言而重写 Java 服务。
...
领域真实冒烟：分别证明核心交易、Chat、Notification、Fulfillment GEO、搜索与 Analytics 专项闭环
...
当前后端、前端、覆盖率、PMD、SpotBugs 和浏览器数字统一见
[验证摘要](verification-summary.md)。阶段数字与 Analytics 的真实 MySQL、Nacos、
RocketMQ、Gateway 故障恢复、重复事件、收入覆盖、偏差注入、审计重建、指标鉴权和
最终清理结论保留在
