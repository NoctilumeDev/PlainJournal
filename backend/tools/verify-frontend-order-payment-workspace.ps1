...
$jars = [ordered]@{
    ...
    catalog = Join-Path $script:backendRoot `
        'services/catalog-service/target/catalog-service.jar'
    inventory = Join-Path $script:backendRoot `
        'services/inventory-service/target/inventory-service.jar'
    marketing = Join-Path $script:backendRoot `
        'services/marketing-service/target/marketing-service.jar'
    trade = Join-Path $script:backendRoot `
        'services/trade-service/target/trade-service.jar'
    payment = Join-Path $script:backendRoot `
        'services/payment-service/target/payment-service.jar'
    fulfillment = Join-Path $script:backendRoot `
        'services/fulfillment-service/target/fulfillment-service.jar'
