...
function Stop-AllApplications {
    ...
    $expectedByPort = @{
        ...
        18108 = 'chat-service-0.1.0-SNAPSHOT.jar'
        18118 = 'chat-service-0.1.0-SNAPSHOT.jar'
        18128 = 'chat-service-0.1.0-SNAPSHOT.jar'
    }
...
"@)
    ...
    $jvmResidue = @(Get-CimInstance Win32_Process |
            Where-Object {
                ...
                (
                    $_.CommandLine -like '*chat-service-0.1.0-SNAPSHOT.jar*' -or
                    $_.CommandLine -like '*ecommerce-gateway-0.1.0-SNAPSHOT.jar*'
                )
            }).Count
