...
function Write-ProxyEvidence {
    ...
    $match = [regex]::Match(
        ...
        '^/api/v1/trade/orders/(?<orderNo>[^/]+)/cancel$')
    ...
    while ($listener.IsListening) {
        ...
        try {
            ...
            [Console]::Error.WriteLine(
                "Trade cancel response-loss proxy request failed: $($_.Exception)")
            if ($context.Response.OutputStream.CanWrite) {
                $message = [Text.Encoding]::UTF8.GetBytes('trade cancellation fault proxy unavailable')
