...
function Write-ProxyEvidence {
    ...
    $evidence = [ordered]@{
        ...
        requestBodySha256 = Get-Sha256 -Bytes $RequestBody
        upstreamStatus = [int]$UpstreamResponse.StatusCode
        upstreamResponseBytes = $ResponseBody.Length
        upstreamResponseSha256 = Get-Sha256 -Bytes $ResponseBody
    }
    ...
    while ($listener.IsListening) {
        ...
        try {
            ...
            $dropResponse = (Test-Path -LiteralPath $ArmFile) -and
                ...
                $context.Request.Url.AbsolutePath -eq '/api/v1/payment/payments' -and
            ...
            [Console]::Error.WriteLine(
                "Payment create response-loss proxy request failed: $($_.Exception)")
            if ($context.Response.OutputStream.CanWrite) {
                $message = [Text.Encoding]::UTF8.GetBytes('payment creation fault proxy unavailable')
