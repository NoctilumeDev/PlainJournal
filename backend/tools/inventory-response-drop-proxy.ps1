...
function Write-ProxyEvidence {
    param(
        ...
        [Parameter(Mandatory)][byte[]]$RequestBody,
    ...
    if ($RequestBody.Length -gt 0) {
        try {
            $payload = [Text.Encoding]::UTF8.GetString($RequestBody) | ConvertFrom-Json
            $reservationNo = [string]$payload.reservationNo
        }
        catch {
            $reservationNo = $null
        }
...
try {
    ...
    while ($listener.IsListening) {
        ...
        try {
            ...
            $targetUri = $upstream + $context.Request.RawUrl
            ...
            $dropResponse = (Test-Path -LiteralPath $ArmFile) -and
                ...
                $context.Request.Url.AbsolutePath -eq '/api/v1/inventory/internal/reservations'
            ...
            [Console]::Error.WriteLine(
                "Inventory response-loss proxy request failed: $($_.Exception)")
            if ($context.Response.OutputStream.CanWrite) {
                $message = [Text.Encoding]::UTF8.GetBytes('inventory fault proxy unavailable')
