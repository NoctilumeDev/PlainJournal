CREATE TABLE consumer_failure (
    ...
    CONSTRAINT ck_inventory_consumer_failure_attempts CHECK (attempts > 0)
);

CREATE INDEX idx_inventory_consumer_failure_status ON consumer_failure (status, last_failed_at);
