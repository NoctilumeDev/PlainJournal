...
class OutboxLeaseIntegrationTest {
    ...
    private static final String AGGREGATE_TYPE = "InventoryOutboxLeaseTest";
    ...
    void fencesExpiredOwnerAndDoesNotExposeAggregateSuccessorEarly() {
        String aggregateId = "inventory-lease-" + UUID.randomUUID();
        Long firstId = insertEvent(aggregateId, 1, BASE_TIME);
        Long secondId = insertEvent(aggregateId, 2, BASE_TIME.plusSeconds(1));
        ...
        assertThat(outboxMapper.claim(
                firstId, "inventory-owner-a", 0, BASE_TIME.plusSeconds(2), firstLeaseUntil,
                BASE_TIME.plusSeconds(2))).isEqualTo(1);
        assertThat(publishableIds(BASE_TIME.plusSeconds(3))).isEmpty();

        assertThat(outboxMapper.markPublished(
                firstId, "inventory-owner-a", firstLeaseUntil.plusMillis(1))).isZero();
        assertThat(outboxMapper.markFailed(
                firstId, "inventory-owner-a", firstLeaseUntil.plusSeconds(1),
        ...
        assertThat(outboxMapper.claim(
                firstId, "inventory-owner-b", 0, recoveredAt, recoveredAt.plusSeconds(30), recoveredAt))
                .isEqualTo(1);
        assertThat(outboxMapper.markPublished(firstId, "inventory-owner-b", recoveredAt.plusSeconds(1)))
                .isEqualTo(1);
    ...
    void rejectsAStaleSelectionAfterARescheduledAttemptBecomesDue() {
        String aggregateId = "inventory-stale-selection-" + UUID.randomUUID();
        Long eventId = insertEvent(aggregateId, 1, BASE_TIME);
        ...
        assertThat(outboxMapper.claim(
                ...
                nextAttemptAt.plusSeconds(31),
                nextAttemptAt.plusSeconds(1))).isZero();
        ...
        assertThat(outboxMapper.claim(
                ...
                nextAttemptAt.plusSeconds(1),
                nextAttemptAt.plusSeconds(31),
                nextAttemptAt.plusSeconds(1))).isEqualTo(1);
    }

    private Long insertEvent(String aggregateId, int version, Instant createdAt) {
        ...
        jdbcTemplate.update("""
                INSERT INTO outbox_event
                    (id, event_id, event_type, aggregate_type, aggregate_id, aggregate_version,
                     ...
                     claim_until, last_error, published_at, created_at, updated_at)
                VALUES
                    (?, ?, 'InventoryLeaseEvent', ?, ?, ?, '{}', 'PENDING', 0, ?,
                     NULL, NULL, NULL, NULL, NULL, ?, ?)
                """,
                id, UUID.randomUUID().toString(), AGGREGATE_TYPE, aggregateId, version,
                createdAt, createdAt, createdAt);
        return id;
    }

    private List<Long> publishableIds(Instant now) {
