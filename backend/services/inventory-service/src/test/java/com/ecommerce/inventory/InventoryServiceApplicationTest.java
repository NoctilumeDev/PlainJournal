...
class InventoryServiceApplicationTest {
    ...
    InventoryServiceApplicationTest(
            MockMvc mockMvc,
            @Qualifier(InventorySchedulingConfig.DEFAULT_SCHEDULER)
            ThreadPoolTaskScheduler defaultScheduler,
            @Qualifier(InventorySchedulingConfig.OUTBOX_SCHEDULER)
            ThreadPoolTaskScheduler outboxScheduler,
            @Qualifier(InventorySchedulingConfig.CONTROL_SCHEDULER)
    ...
    @Test
    void outboxSchedulerRunsWhileConsumerSchedulerIsBlocked() throws Exception {
        ...
        try {
            ...
            assertThat(outboxThread.get()).startsWith("inventory-outbox-scheduling-");
            assertThat(controlThread.get()).startsWith("inventory-control-scheduling-");
