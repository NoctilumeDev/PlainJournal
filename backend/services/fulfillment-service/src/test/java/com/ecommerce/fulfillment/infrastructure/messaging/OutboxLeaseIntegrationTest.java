...
class OutboxLeaseIntegrationTest {
    ...
    private static final String AGGREGATE_TYPE = "FulfillmentOutboxLeaseTest";
    ...
    void fencesExpiredOwnerAndDoesNotExposeAggregateSuccessorEarly() {
        String aggregateId = "fulfillment-lease-" + UUID.randomUUID();
        ...
        assertThat(outboxMapper.claim(
                firstId, "fulfillment-owner-a", 0, BASE_TIME.plusSeconds(2), firstLeaseUntil)).isEqualTo(1);
        assertThat(publishableIds(BASE_TIME.plusSeconds(3))).isEmpty();

        assertThat(outboxMapper.markPublished(
                firstId, "fulfillment-owner-a", firstLeaseUntil.plusMillis(1))).isZero();
        assertThat(outboxMapper.markFailed(
                firstId, "fulfillment-owner-a", firstLeaseUntil.plusSeconds(1),
        ...
        assertThat(outboxMapper.claim(
                firstId, "fulfillment-owner-b", 0, recoveredAt, recoveredAt.plusSeconds(30))).isEqualTo(1);
        assertThat(outboxMapper.markPublished(
                firstId, "fulfillment-owner-b", recoveredAt.plusSeconds(1))).isEqualTo(1);
    ...
    void rejectsAStaleSelectionBeforeAndAfterTheRescheduledAttemptBecomesDue() {
        String aggregateId = "fulfillment-stale-selection-" + UUID.randomUUID();
    ...
    private String insertEvent(String aggregateId, int version, Instant createdAt) {
        ...
        jdbcTemplate.update("""
                ...
                VALUES
                    (?, 'FulfillmentLeaseEvent', ?, ?, ?, '{}', 'PENDING', 0, ?,
