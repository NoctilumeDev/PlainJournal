...
class FulfillmentServiceApplicationTest {
    ...
    @Autowired
    FulfillmentServiceApplicationTest(
            MockMvc mockMvc,
            @Qualifier(FulfillmentSchedulingConfig.DEFAULT_SCHEDULER)
            ThreadPoolTaskScheduler defaultScheduler,
            @Qualifier(FulfillmentSchedulingConfig.OUTBOX_SCHEDULER)
            ThreadPoolTaskScheduler outboxScheduler,
            @Qualifier(FulfillmentSchedulingConfig.CONTROL_SCHEDULER)
    ...
    void outboxAndControlSchedulersRunWhileBothConsumerThreadsAreBlocked() throws Exception {
        ...
        try {
            ...
            assertThat(outboxThread.get()).startsWith("fulfillment-outbox-scheduling-");
            assertThat(controlThread.get()).startsWith("fulfillment-control-scheduling-");
