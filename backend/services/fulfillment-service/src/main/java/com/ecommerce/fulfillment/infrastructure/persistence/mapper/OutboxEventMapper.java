...
public interface OutboxEventMapper extends BaseMapper<OutboxEventEntity> {
    ...
    @Update("""
            ...
            WHERE status = 'PUBLISHING'
              AND (claim_until IS NULL OR claim_until <= #{expiredAt})
            """)
    int resetStaleClaims(@Param("expiredAt") Instant expiredAt, @Param("now") Instant now);

    @Select("""
            SELECT candidate.* FROM outbox_event candidate
    ...
    @Update("""
            UPDATE outbox_event SET status = 'PUBLISHING', claimed_at = #{claimedAt},
    ...
    @Update("""
            UPDATE outbox_event SET status = 'PUBLISHED', published_at = #{now}, claimed_at = NULL,
    ...
    @Update("""
            UPDATE outbox_event SET status = 'PENDING', attempts = attempts + 1,
                next_attempt_at = #{nextAttemptAt}, claimed_at = NULL,
                claim_owner = NULL, claim_until = NULL, last_error = #{error}, updated_at = #{now}
            WHERE id = #{id} AND status = 'PUBLISHING' AND claim_owner = #{owner}
              AND claim_until > #{now}
            """)
    int markFailed(
