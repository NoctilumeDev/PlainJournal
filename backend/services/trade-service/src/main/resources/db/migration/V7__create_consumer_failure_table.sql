CREATE TABLE consumer_failure (
    ...
    CONSTRAINT ck_trade_consumer_failure_attempts CHECK (attempts > 0)
);

CREATE INDEX idx_trade_consumer_failure_status ON consumer_failure (status, last_failed_at);
