...
import com.ecommerce.platform.common.observability.ConsumerFailureStore;
...
public interface ConsumerFailureMapper extends ConsumerFailureStore {
    ...
    @Update("""
            ...
            SET attempts = GREATEST(attempts, #{attempts}),
                ...
                next_attempt_at = CASE
                    ...
                    WHEN next_attempt_at <= #{nextAttemptAt} THEN next_attempt_at
                    ELSE #{nextAttemptAt}
                END,
                claimed_at = NULL,
                claim_owner = NULL,
    ...
    @Select("""
            SELECT message_id, consumer_group, raw_payload, attempts, next_attempt_at
            ...
            WHERE status = 'RETRYING'
              ...
              AND (
                 ...
                 OR consumer_group = #{deliveryGroup}
              )
            ORDER BY next_attempt_at, message_id
            LIMIT #{limit}
            """)
    List<ConsumerFailureRetryEntity> selectRetryable(
            @Param("dispatcherGroup") String dispatcherGroup,
            @Param("deliveryGroup") String deliveryGroup,
            @Param("now") Instant now,
