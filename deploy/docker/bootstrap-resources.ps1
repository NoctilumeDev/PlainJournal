...
"@
    ...
    $databaseSql | docker exec -i -e "MYSQL_PWD=$RootPassword" ecom-mysql mysql -uroot
...
$null = Ensure-EnvValue -Name 'INTERNAL_SERVICE_TOKEN' -Value (New-HexSecret -ByteLength 32)
...
foreach ($topic in $topics) {
    $result = docker exec ecom-rocketmq-broker sh mqadmin updateTopic -n ecom-rocketmq-namesrv:9876 -c EcommerceCluster -t $topic -r 4 -w 4 2>&1
...
docker exec ecom-minio sh -c $bucketCommand
if ($LASTEXITCODE -ne 0) {
    throw 'MinIO bucket initialization failed.'
}

Write-Host 'Service databases/secrets, Nacos configuration, RocketMQ topics, and MinIO buckets are ready.'
